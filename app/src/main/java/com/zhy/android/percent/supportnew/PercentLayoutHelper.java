...
public class PercentLayoutHelper {
    ...
    private static int sWidthScreen;
    private static int sHeightScreen;
    private static boolean sIsScreenPortrait = true;

    public PercentLayoutHelper(ViewGroup host) {
        mHost = host;
        getScreenSize(host.getContext());
    }
...
//            sWidthScreen = outMetrics.widthPixels;
//            sHeightScreen = outMetrics.heightPixels;
//            return;
//        } else {
...
//                sHeightScreen = outMetrics.heightPixels + navigationHeight;
//            } else {
//                sHeightScreen = outMetrics.heightPixels;
//            }
//        }
//    }
    ...
    private static void getScreenSize(Context context) {
        if (sWidthScreen == 0 || sHeightScreen == 0) {
            int[] arr = ScreenUtils.getScreenWeightAndHeightFull((Activity) context, null);
            sWidthScreen = arr[2];
            sHeightScreen = arr[3];
            sIsScreenPortrait = ScreenUtils.isScreenOriatationPortrait(context);
        } else {
            boolean screenOriatationPortrait = ScreenUtils.isScreenOriatationPortrait(context);
            if (screenOriatationPortrait != sIsScreenPortrait) {
                int[] arr = ScreenUtils.getScreenWeightAndHeightFull((Activity) context, null);
                sWidthScreen = arr[2];
                sHeightScreen = arr[3];
                sIsScreenPortrait = screenOriatationPortrait;
            }
        }
    }
    ...
    private void invokeMethod(String methodName, int widthHint, int heightHint, View view, Class clazz,
                              PercentLayoutInfo.PercentVal percentVal) throws NoSuchMethodException,
